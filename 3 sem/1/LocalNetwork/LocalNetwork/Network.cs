...
namespace LocalNetwork
{
    public class Network
    {
        ...
        public Network(string path)
        {
            ...
            this.matrix = new int[this.numberOfComputers][];
            ...
            for (int i = 0; i < this.numberOfComputers; ++i)
            {
                this.matrix[i] = new int[this.numberOfComputers];
                for (int j = 0; j < this.numberOfComputers; ++j)
                {
                    matrix[i][j] = matr[count][0] - 48;
        ...
        public void Step()
        {
            ...
            this.Traverse(0, false, ref  visited);
        }
        ...
        private void Traverse(int numberOfComputer, bool isInfection, ref bool[] visited)
        {
            ...
            for (int i = 0; i < this.numberOfComputers; ++i)
            {
                if (!visited[i] && (this.matrix[numberOfComputer][i] == 1))
                {
                    this.Traverse(i, this.listOfComputers[numberOfComputer].Virus, ref visited);
                }
            }
        }
    }
